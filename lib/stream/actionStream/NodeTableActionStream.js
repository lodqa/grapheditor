...
export default class extends ActionReadable {
  ...
  _bindComponent(selector, push) {
    ...
    // nodes
    component.ractive.observe('nodes.*.label', (newValue, oldValue, keypath, index) => updateLabel(component, index, push))
    ...
    // terms
    component.ractive.observe('nodes.*.terms.*.enable nodes.*.terms.*.value', (newValue, oldValue, keypath, nodeIndex) => updateTerm(component, nodeIndex, push))
    container.on('keyup', '.term input', e => {
      if (e.key === 'Enter' && e.target.value) {
