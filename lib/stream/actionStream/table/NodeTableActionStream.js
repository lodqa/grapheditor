...
function updateTerm(component, index, push) {
  if (component.has(index)) {
    ...
    pushUpdateTerm(target.MODEL_NODE, text.id, text.terms, push)
  }
}
...
function delTermButtonClick(e, component, push) {
  ...
  pushUpdateTerm(target.MODEL_NODE, text.id, newTerms, push)
}
...
function addTerm(component, input, push) {
  ...
  if (newValue) {
    ...
    pushUpdateTerm(target.MODEL_NODE, text.id, text.terms.concat({
      enable: true,
      value: newValue
    }), push)
