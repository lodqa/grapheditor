...
<table>
  <tbody>
    {{#texts}}
    <tr data-source-id="{{sourceId}}" data-target-id="{{targetId}}" data-index={{@index}} class="{{#if selected}}selected{{/if}} {{#if hover}}hover{{/if}}">
      <td class="focus">
      </td>
      <td class="text">
        <input value="{{text}}">
...
export default function(selector) {
  ...
  return {
    ...
    getTextId: (element) => {
      ...
      if (trElement) {
        return [trElement.getAttribute('data-source-id'), trElement.getAttribute('data-target-id')]
      }
